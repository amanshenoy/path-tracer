...
namespace optim {
    ...
    bool BVHNode::hit(const core::Ray& r, double t_min, double t_max, utils::hit_details& rec){
    ...
    BVHNode::BVHNode(std::vector<std::shared_ptr<Object>>& objects, size_t start, size_t end, double time0, double time1, double& node_id, bool first) {
        ...
        #pragma omp critical
        utils::show_tree_building(node_id, progress_bar_width, static_cast<int>(objects.size()));
        int axis = 0; 
