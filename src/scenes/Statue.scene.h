...
auto stone = std::make_shared<core::Disney>(glm::vec3(136.0f/255.0f,140.0f/255.0f,141.0f/255.0f), 0.2, 0.8, 0, 0, false);
// core::ImageBG background("res/backgrounds/old_hall_2k.hdr");
lookfrom = glm::vec3(-2, 0.25, -0.75);
lookat = glm::vec3(-.9, .6, 0); 

aperture = 1.0f / 44.0f;
// aperture = 0.0f; 
focal_distance = glm::length(lookfrom - lookat) + 0.75; 
...
std::string mesh_path = "res/objects/lucy.obj";
auto bunny_mesh = std::make_shared<core::Mesh>(mesh_path, stone, true);
...
bunny = std::make_shared<core::RotateY>(bunny, -140);
